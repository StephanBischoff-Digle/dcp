...
public:
    ...
    std::size_t n_unival_subtrees() const {
        std::function<std::tuple<bool, T, std::size_t>(const btree<T>&)>
                recursive_unival_subtree;
        recursive_unival_subtree =
                [&recursive_unival_subtree](const btree<T>& tree)
                -> std::tuple<bool, T, std::size_t> {
            ...
            if (tree.left) {
                auto [child_unival, val, n_unival_sub] =
                        recursive_unival_subtree(*tree.left);
                is_unival = (child_unival && val == tree.value);
                unival_subs += n_unival_sub;
            }
            if (tree.right) {
                auto [child_unival, val, n_unival_sub] =
                        recursive_unival_subtree(*tree.right);
                is_unival = (child_unival && is_unival && val == tree.value);
                unival_subs += n_unival_sub;
            }

            return {is_unival, tree.value,
                    is_unival ? unival_subs + 1 : unival_subs};
        };
